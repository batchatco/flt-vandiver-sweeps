...
| Case II descent runs for `2ℓ ≤ 3p² − 5p + 2`, extending the artifact's `ℓ < p² − p` (Section 10, effective Vandiver) | `bad-certificate/BadCertificate.lean` (Lean, standard axioms; the original read-only check against flt-vandiver `afm-v1`, since folded into flt-vandiver `afm-v2` as `FltVandiver/BadCertificate.lean`) | `bad-certificate/check.log` |
